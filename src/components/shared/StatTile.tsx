...
/**
 * Compact stat card shared by the manager and student dashboards (and mirrors
 * the admin dashboard's hero tile): one `emphasis` tile stands out, the rest
 * stay flat and equal so the row isn't just N identical boxes.
 */
  ...
  return (
    ...
    >
      <CardContent className={`flex items-center justify-between ${emphasis ? 'p-7' : 'p-5'}`}>
        ...
        >
          <Icon className={`h-6 w-6 ${emphasis ? 'text-white' : 'text-blue-600'}`} />
        </div>
      </CardContent>
    </Card>
  );
