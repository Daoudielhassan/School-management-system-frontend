 ...
 * authoritative parse still happens server-side (`InstructorFileParser`,
 * Apache POI/OpenCSV); this is purely a fast local preview.
 */
import * as XLSX from 'xlsx';
import { INSTRUCTOR_UPLOAD_REQUIRED_HEADERS } from '../constants';
...
export async function parseBulkUploadFile(file: File): Promise<BulkUploadPreview> {
  ...
  if (rows.length === 0) {
    return { headers: [], missingHeaders: [...INSTRUCTOR_UPLOAD_REQUIRED_HEADERS], rows: [], totalRows: 0 };
  }

  const [headerRow, ...dataRows] = rows;
  const normalizedHeaders = headerRow.map(normalizeHeader);
  const missingHeaders = INSTRUCTOR_UPLOAD_REQUIRED_HEADERS.filter((h) => !normalizedHeaders.includes(h));
