...
function downloadCredentialsCsv(result: BulkUploadResult) {
  ...
  const header = 'name,email,code,temporaryPassword';
  const lines = successRows.map((r) => `${r.name},${r.email},${r.code ?? ''},${r.temporaryPassword}`);
  const blob = new Blob([[header, ...lines].join('\n')], { type: 'text/csv;charset=utf-8' });
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = `instructor-credentials-${new Date().toISOString().slice(0, 10)}.csv`;
  a.click();
