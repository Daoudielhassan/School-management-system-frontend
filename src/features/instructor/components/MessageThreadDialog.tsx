...
export function MessageThreadDialog({ message, onOpenChange }: MessageThreadDialogProps) {
  ...
  useEffect(() => {
    // Only the receiver can mark a message read — the backend rejects (404)
    // any attempt where the caller isn't the message's receiverId. Opening a
    // message from the "Sent" tab means the current user is the sender, so
    // skip the call there.
