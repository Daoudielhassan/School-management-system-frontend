...
import { INSTRUCTOR_MESSAGES_QUERY_KEY } from '../constants';
...
export function useMyInbox() {
  ...
  return useQuery<MessageResponse[]>({
    queryKey: [...INSTRUCTOR_MESSAGES_QUERY_KEY, 'inbox'],
...
export function useMyUnreadMessageCount() {
  ...
  return useQuery<number>({
    queryKey: [...INSTRUCTOR_MESSAGES_QUERY_KEY, 'unread-count'],
...
export function useMySentMessages() {
  ...
  return useQuery<MessageResponse[]>({
    queryKey: [...INSTRUCTOR_MESSAGES_QUERY_KEY, 'sent', userId],
...
export function useMessageThread(parentMessageId?: string) {
  ...
  return useQuery<MessageResponse[]>({
    queryKey: [...INSTRUCTOR_MESSAGES_QUERY_KEY, 'thread', parentMessageId],
...
function useInvalidateMyMessages() {
  ...
  return () => queryClient.invalidateQueries({ queryKey: INSTRUCTOR_MESSAGES_QUERY_KEY });
}
