...
import { INSTRUCTOR_NOTIFICATIONS_QUERY_KEY } from '../constants';
...
export function useMyNotifications() {
  ...
  return useQuery<NotificationResponse[]>({
    queryKey: INSTRUCTOR_NOTIFICATIONS_QUERY_KEY,
...
export function useMyUnreadNotificationCount() {
  ...
  return useQuery<number>({
    queryKey: [...INSTRUCTOR_NOTIFICATIONS_QUERY_KEY, 'unread-count'],
...
function useInvalidateMyNotifications() {
  ...
  return () => queryClient.invalidateQueries({ queryKey: INSTRUCTOR_NOTIFICATIONS_QUERY_KEY });
}
